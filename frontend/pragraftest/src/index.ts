 ...
 * Import types
 */
import { pragraftestData, pragraftestConfig } from './types';
import { API, BlockAPI, BlockTool } from '@editorjs/editorjs';

/**
 * pragraftest Tool for Editor.js
 */
export default class pragraftest implements BlockTool {
  /**
   ...
   */
  constructor({ data, config, api, block, readOnly }: { data: pragraftestData, config: pragraftestConfig, api: API, block: BlockAPI, readOnly: boolean }) {
    this.data = data;
    this.config = config;
    this.api = api;
    this.block = block;
  ...
  render() {
    ...
    this.nodes.wrapper.classList.add(styles['pragraftest-tool']);
   ...
   * 
   * @returns {pragraftestData} saved data
   */
  save(): pragraftestData {
    return {};
  }
   ...
   * 
   * @param {pragraftestData} savedData
   ...
   * 
   * @param {pragraftestData} data - data of second Block
   * @returns {pragraftestData} - merged data
   */
  // merge() {} 
